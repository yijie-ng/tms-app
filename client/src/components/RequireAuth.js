...
const RequireAuth = ({ allowedRoles }) => {
    ...
    return (
        auth?.userRole && allowedRoles?.includes(auth?.userRole)
            ...
            : auth?.username
                ...
                : <Navigate to="/login" state={{ from: location }} replace /> // if not logged in, redirect user to login page from where they came from
    )
}
